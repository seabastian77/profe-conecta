...
// Reescribe el correo del seed al dominio que la aplicación admite.
function correoInstitucional(correo) {
  return correo.split('@')[0].toLowerCase() + DOMINIO_INSTITUCIONAL;
}

// Crea o verifica las tablas, índices, claves foráneas y datos base.
async function migrar() {
  ...
  await c.query(`
    ...
    -- Registra los intentos de login para el bloqueo por fuerza bruta.
  ...
  // Crea los índices de las consultas más frecuentes.
  await c.query(`
  ...
  // Inserta el periodo activo y la configuración de reglas de negocio.
  await c.query(`
...
// Inserta el catálogo de asignaturas de ejemplo.
...
// Crea 10 docentes y 10 estudiantes de ejemplo solo si aún no hay ninguno.
async function sembrarUsuarios(c) {
  ...
  // Usa 10 rondas: son cuentas de ejemplo con contraseña pública.
...
// CASCADE en registros operativos y SET NULL en auditoría al borrar un usuario.
...
// Ajusta cada clave foránea a su acción de borrado esperada (idempotente).
...
// Permite correr la migración directamente con node src/config/migrate.js.
